  ...
  realsense2_camera_msgs__msg__RGBD__Sequence * output)
{
  ...
  if (output->capacity < input->size) {
    ...
    realsense2_camera_msgs__msg__RGBD * data =
      (realsense2_camera_msgs__msg__RGBD *)realloc(output->data, allocation_size);
    if (!data) {
      return false;
    }
    for (size_t i = output->capacity; i < input->size; ++i) {
      if (!realsense2_camera_msgs__msg__RGBD__init(&data[i])) {
        /* free currently allocated and return false */
        for (; i-- > output->capacity; ) {
          realsense2_camera_msgs__msg__RGBD__fini(&data[i]);
        }
        free(data);
        return false;
      }
